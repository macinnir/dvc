...
// MSSQL contains functionality for interacting with a server
type MSSQL struct {
	config *lib.ConfigDatabase
}

func NewMSSQL(config *lib.ConfigDatabase) *MSSQL {
	return &MSSQL{
		config: config,
	}
}
...
func (ss *MSSQL) FetchDatabases(server *schema.Server) (map[string]*schema.Database, error) {

	var e error
	var databases = map[string]*schema.Database{}
	var rows *sql.Rows

	if rows, e = server.Connection.Query(`SELECT 
			name, 
...
// FetchDatabases fetches a set of database names from the target server
// populating the Databases property with a map of Database objects
func (ss *MSSQL) fetchDatabaseMeta(server *schema.Server, schema *schema.Database) (e error) {

	// var rows *sql.Rows

	// if rows, e = server.Connection.Query(fmt.Sprintf(`SELECT
	// 	DEFAULT_CHARACTER_SET_NAME,
	// 	DEFAULT_COLLATION_NAME
	// 	FROM information_schema.SCHEMATA
	// 	WHERE
	// 	SCHEMA_NAME = '%s'`, schema.Name)); e != nil {
	// 	return
	// }
...
func (ss *MSSQL) UseDatabase(server *schema.Server, databaseName string) (e error) {
...
func (ss *MSSQL) FetchDatabase(schemaName string, server *schema.Server, databaseName string) (*schema.Database, error) {
...
func (ss *MSSQL) fetchDatabaseTables(schemaName string, server *schema.Server, databaseName string) (tables map[string]*schema.Table, e error) {
...
func (ss *MSSQL) FetchTableColumns(server *schema.Server, databaseName string, tableName string) (columns map[string]*schema.Column, e error) {
...
func (ss *MSSQL) CreateChangeSQL(localSchema *schema.Schema, remoteSchema *schema.Schema, databaseName string) *schema.SchemaComparison {
...
// func (ss *MSSQL) FetchEnums(server *schema.Server) (enums map[string][]map[string]interface{}) {
...
func (ss *MSSQL) FetchEnum(server *schema.Server, tableName string) (objects []map[string]interface{}) {
